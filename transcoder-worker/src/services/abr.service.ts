...
/**
 * Generate ABR HLS ladder (1080p, 720p, 480p, 360p) from a single input.
 * Decode once, scale to four renditions, output HLS with .ts segments.
 * Output: outputDir/master.m3u8, outputDir/1080p/, 720p/, 480p/, 360p/
 */
export async function generateAbrHls(
  ...
  await fs.mkdir(path.join(outputDir, '3'), { recursive: true });
  ...
  // Rename 0 -> 1080p, 1 -> 720p, 2 -> 480p, 3 -> 360p
  const renames = [
    [path.join(outputDir, '0'), path.join(outputDir, '1080p')],
    [path.join(outputDir, '1'), path.join(outputDir, '720p')],
    [path.join(outputDir, '2'), path.join(outputDir, '480p')],
    [path.join(outputDir, '3'), path.join(outputDir, '360p')],
  ];
  ...
  // Write master.m3u8 referencing 1080p, 720p, 480p, 360p
